...
-- Acceptable values for @TableName are All, ContactAddresses, ContactNotes, ContactPhoneNumbers, and ContactRoles.
...
IF (@TableName IN ('All', 'ContactNotes'))
 BEGIN
	SELECT @NoteCount = COUNT(1)
		FROM dbo.ContactNotes CN
...
IF (@TableName IN ('All', 'ContactPhoneNumbers'))
 BEGIN
	SELECT @PhoneNumberCount = COUNT(1)
